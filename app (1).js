...
function crearTarjetaHabilitada(materia) {
  ...
  const guardar = () => {
    ...
    if (resultado.error) {
      error.classList.remove("nota-info");
      error.textContent = resultado.error;
      return;
    }
    if (resultado.num >= 4) {
      // Nota aprobatoria: la materia se aprueba y sale de "Habilitadas",
      // animando la transición hacia la correlativa que se desbloquee.
      animarAprobacionDesdeHabilitada(materia, card, resultado.num);
    } else {
      // Nota reprobatoria: se registra como intento desaprobado, pero la
      // materia SIGUE habilitada, así que la tarjeta no se mueve de acá.
      registrarNota(materia.codigo, resultado.num, { skipRender: true });
      renderStats();
      renderProximas();
      renderAprobadas();
      renderDesaprobados();
      renderPlanCompleto();
      input.value = "";
      error.textContent = "Intento registrado en \"Finales desaprobados\". La materia sigue habilitada.";
      error.classList.add("nota-info");
    }
  };
